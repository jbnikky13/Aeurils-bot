...
def _net(pnl_pct): return float(pnl_pct or 0)-ROUND_TRIP_FEE_PCT
...
def _group(rows, key_fn):
    ...
    for r in rows:
        key=key_fn(r)
        win=str(r["outcome"] or "").upper() in {"WIN_TP1","WIN_TP2","WIN_RUNNER","WIN"}
        g=groups[key]; g[0]+=1; g[1]+=int(win); g[2]+=_net(r["pnl_pct"])
...
def calibration():
    ...
    pnl=sum(_net(r["pnl_pct"]) for r in closed)
    return {"closed":len(closed),"wins":wins,"tp1":tp1,"tp2":tp2,"runner":runner,"sl":sl,"expired":expired,
            ...
            "runner_rate_pct":100*runner/len(closed) if closed else 0.0,
            ...
            "by_confluence_tier":_group(closed,lambda r:(r["confluence_tier"] if "confluence_tier" in r.keys() and r["confluence_tier"] else "UNKNOWN")),
            "by_source":_group(closed,lambda r:(r["signal_source"] if "signal_source" in r.keys() and r["signal_source"] else "PRIMARY"))}
...
def format_report():
    ...
    if n:
        lines += [f"Win rate: {a['win_rate_pct']:.1f}%",f"TP1 exits: {a['tp1_rate_pct']:.1f}%",f"TP2 exits: {a['tp2_rate_pct']:.1f}%",f"Runner exits: {a['runner_rate_pct']:.1f}%",f"SL exits: {a['sl_rate_pct']:.1f}%",f"24h expiries: {a['expiry_rate_pct']:.1f}%",f"Total paper P&L (net of {a['fee_assumption_pct']:.2f}% round-trip fee): {a['pnl_pct']:.2f}%",f"Average trade: {a['avg_pnl_pct']:.2f}%"]
    ...
    lines += ["","By confluence tier:"]
    lines += [f"• {k}: {n} trades | {w:.1f}% wins | {p:.2f}% P&L" for k,n,w,p in a["by_confluence_tier"]] or ["• No closed observations yet."]
    lines += ["","By signal source:"]
    lines += [f"• {k}: {n} trades | {w:.1f}% wins | {p:.2f}% P&L" for k,n,w,p in a["by_source"]] or ["• No closed observations yet."]
