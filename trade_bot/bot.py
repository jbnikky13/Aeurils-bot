...
def _fmt_discovery(title, items):
    lines=[title]
    for i,x in enumerate(items,1): lines.append(f"{i}. {x['name']} ({x['symbol'].upper()}) — score {x['score']}/100 | MCap ${x['market_cap']:,.0f} | Vol ${x['volume_24h']:,.0f}")
...
def _fmt_gems(items):
    ...
    for i,x in enumerate(items,1): lines.append(f"{i}. {x.name} ({x.symbol}) — {x.status} | Gem {x.final_score:.1f}/100 | {'; '.join(x.reasons[:3])}")
...
async def gems(update,context):
    try:await update.message.reply_text(_fmt_gems(await scan_candidates(limit=int(os.getenv("GEM_VALIDATION_LIMIT","5"))))[:3900])
