...
def direction_from_components(technical_bias:float,whale_bias:float)->Direction:
    bias=.65*technical_bias+.35*whale_bias
    if bias>=.20:return 'LONG'
    if bias<=-.20:return 'SHORT'
    return 'WAIT'
...
def build_setup(symbol:str,asset_type:str,price:float,technical_score:int,whale_score:int,sentiment_score:float,technical_bias:float,whale_bias:float,atr:float,market_regime:str='UNKNOWN',ema20:float|None=None,recent_high:float|None=None,recent_low:float|None=None,rsi:float|None=None)->Signal:
    ...
    if direction=='WAIT' or score<60 or atr<=0:return Signal(symbol,asset_type,'WAIT',score,None,None,None,None,None,None,technical_score,whale_score,sentiment_score,['No sufficiently strong directional setup.'],'Wait for confirmation.',market_regime,whale_bias=whale_bias)
    ...
    if rsi is not None:
        if direction=='LONG' and rsi>OVERBOUGHT_RSI:return Signal(symbol,asset_type,'WAIT',score,None,None,None,None,None,None,technical_score,whale_score,sentiment_score,[f'Entry rejected: RSI {rsi:.1f} is overbought for a new long.'],'Wait for RSI to cool and price to retest support.',market_regime,whale_bias=whale_bias,entry_status='OVEREXTENDED_MOMENTUM',entry_quality=entry_quality)
        if direction=='SHORT' and rsi<OVERSOLD_RSI:return Signal(symbol,asset_type,'WAIT',score,None,None,None,None,None,None,technical_score,whale_score,sentiment_score,[f'Entry rejected: RSI {rsi:.1f} is oversold for a new short.'],'Wait for RSI to recover and price to retest resistance.',market_regime,whale_bias=whale_bias,entry_status='OVEREXTENDED_MOMENTUM',entry_quality=entry_quality)
    zone=min(.0025,max(.00075,.15*atr/price)); entry_low,entry_high=price*(1-zone),price*(1+zone)
    ...
    # Reject trades where the nearest recent structure is too close to TP1.
    if direction=='LONG' and recent_high is not None and recent_high>price and recent_high<tp1 and (recent_high-price)<.75*atr:
        return Signal(symbol,asset_type,'WAIT',score,None,None,None,None,None,None,technical_score,whale_score,sentiment_score,[f'Entry rejected: nearby resistance at {recent_high:.6g} leaves insufficient room for TP1.'],'Wait for a clean breakout/retest of resistance.',market_regime,whale_bias=whale_bias,entry_status='BLOCKED_BY_RESISTANCE',entry_quality=entry_quality)
    if direction=='SHORT' and recent_low is not None and recent_low<price and recent_low>tp1 and (price-recent_low)<.75*atr:
        return Signal(symbol,asset_type,'WAIT',score,None,None,None,None,None,None,technical_score,whale_score,sentiment_score,[f'Entry rejected: nearby support at {recent_low:.6g} leaves insufficient room for TP1.'],'Wait for a clean breakdown/retest of support.',market_regime,whale_bias=whale_bias,entry_status='BLOCKED_BY_SUPPORT',entry_quality=entry_quality)
