...
async def safe_setup(symbol: str, asset_type: str):
    try:
        return await crypto_setup(symbol) if asset_type == "crypto" else stock_setup(symbol)
...
async def setup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = await safe_setup(symbol, "crypto" if symbol.endswith("USDT") else "stock")
...
async def crypto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    results = [await safe_setup(s, "crypto") for s in CRYPTO]
    await update.message.reply_text("\n\n".join(format_signal(x) if not isinstance(x, str) else x for x in results)[:3900])

async def stocks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    results = [await safe_setup(s, "stock") for s in STOCKS]
    await update.message.reply_text("\n\n".join(format_signal(x) if not isinstance(x, str) else x for x in results)[:3900])
