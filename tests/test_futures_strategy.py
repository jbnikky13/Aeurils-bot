...
class FuturesStrategyTests(unittest.TestCase):
    def _continuation_df(self):
        n=35
        closes=[120-i*0.45 for i in range(n-1)] + [104.0]
        highs=[c+1 for c in closes]; lows=[c-1 for c in closes]; volumes=[100]*34+[220]
        ema20=[130-i*0.5 for i in range(n-1)]+[108]
        ema50=[140-i*0.4 for i in range(n-1)]+[112]
        ema200=[160-i*0.2 for i in range(n-1)]+[120]
        rows=[]
        for i in range(n):
            rows.append({"close":closes[i],"high":highs[i],"low":lows[i],"volume":volumes[i],"ema20":ema20[i],"ema50":ema50[i],"ema200":ema200[i],"rsi":28,"adx":32,"macd_hist":-1,"atr":4,"volume_ratio":1.0 if i<n-1 else 2.2})
        # Make the current close a clean break of the previous 20-candle low.
    ...
    def test_continuation_detects_strong_short_even_with_oversold_rsi(self):
        flow={"oi_change_pct":3.0,"taker_ratio":0.80,"liquidation_pressure":True}
        result=assess(self._continuation_df(),"SHORT",flow)
        self.assertIn(result.mode,{"BREAKOUT","CONTINUATION"})
        self.assertGreaterEqual(result.score,70)
        self.assertTrue(result.trend_aligned)
        self.assertTrue(result.momentum_confirmed)
        self.assertTrue(result.flow_confirmed)
    ...
    def test_continuation_bypasses_normal_rsi_and_extension_veto(self):
        ...
        self.assertTrue(signal.runner_enabled)
        self.assertEqual(signal.entry_status,"READY_CONTINUATION")
    ...
    def test_futures_universe_validation_is_fail_closed(self):
        self.assertTrue(is_valid_binance_usdt_perpetual("BRUSDT",["BRUSDT","BTCUSDT"]))
        self.assertFalse(is_valid_binance_usdt_perpetual("BRUSDT",["BTCUSDT"]))

    def test_futures_flow_uses_futures_depth(self):
        responses={
            "/fapi/v1/depth":{"bids":[[100,10]],"asks":[[100,5]]},
            "/fapi/v1/openInterest":{"openInterest":"100"},
            "/futures/data/openInterestHist":[{"sumOpenInterest":"100"},{"sumOpenInterest":"104"},{"sumOpenInterest":"104"}],
            "/fapi/v1/klines":[[0,"100","101","99","100","1",0,0,0,0,0,0],[1,"100","101","95","96","1",0,0,0,0,0,0]],
            "/fapi/v1/fundingRate":[{"fundingRate":"0.0001"}],
            "/futures/data/takerlongshortRatio":[{"buySellRatio":"0.80"}],
            "/fapi/v1/allForceOrders":[],
        }
        with patch("trade_bot.binance_flow._get",side_effect=lambda path,params=None:responses[path]):
            result=snapshot("BRUSDT","SHORT")
    ...
    def test_runner_keeps_trade_open_after_tp1(self):
        ...
        try:
            ...
            with sqlite3.connect(tmp.name) as con:
                row=con.execute("SELECT status,remaining_pct,tp1_hit_at FROM paper_trades WHERE signal_id=?",(sid,)).fetchone()
