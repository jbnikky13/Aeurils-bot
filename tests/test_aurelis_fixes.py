...
class AurelisFixTests(unittest.TestCase):
    ...
    def test_weighted_gate_allows_a_setup_without_six_confluences(self):
        class Signal:
            direction = "LONG"; score = 70; technical_score = 0
            entry_low = 100; entry_high = 101; stop_loss = 98; take_profit_1 = 105; risk_reward = 2
    ...
    def test_weighted_gate_keeps_bad_risk_non_actionable(self):
        class Signal:
            direction = "LONG"; score = 90; technical_score = 90
            entry_low = 100; entry_high = 101; stop_loss = 99; take_profit_1 = 101; risk_reward = .5
    ...
    def test_paper_close_syncs_setup_and_calibration(self):
        journal.init_db()
        class Signal:
            symbol = "TESTUSDT"; asset_type = "crypto"; direction = "SHORT"; score = 66
            entry_low = 99; entry_high = 101; stop_loss = 105; take_profit_1 = 95; take_profit_2 = 90
            risk_reward = 2; technical_score = 80; whale_score = 50; sentiment_score = 50
            market_regime = "BEARISH"; gemini_confidence = None; gemini_decision = "UNAVAILABLE"
            gemini_rationale = "Gemini unavailable"; whale_bias = 0
        signal_id = journal.record_setup(Signal())
        paper_trader.open_paper_trade(signal_id, "TESTUSDT", "SHORT", 100, 105, 95, 90,
                                      final_score=66, market_regime="BEARISH", gemini_decision="UNAVAILABLE",
                                      gemini_confidence=None, gemini_available=0)
    ...
    def test_hourly_range_resolves_tp_from_completed_candle(self):
        journal.init_db()
        sid = journal.record_setup(type("S", (), {
            "symbol": "RANGEUSDT", "asset_type": "crypto", "direction": "SHORT", "score": 80,
            "entry_low": 99, "entry_high": 101, "stop_loss": 105, "take_profit_1": 95,
            "take_profit_2": 90, "risk_reward": 2, "technical_score": 80, "whale_score": 50,
            "sentiment_score": 50, "market_regime": "BEARISH"
        })())
        paper_trader.open_paper_trade(sid, "RANGEUSDT", "SHORT", 100, 105, 95, 90)
        self.assertEqual(paper_trader.mark_candle("RANGEUSDT", {
            "time": datetime.now(timezone.utc).isoformat(), "high": 102, "low": 94, "close": 99
        }), 1)
        with __import__("sqlite3").connect(self.tmp.name) as con:
    ...
    def test_same_hour_stop_and_target_is_conservative(self):
        ...
        journal.init_db()
        sid = journal.record_setup(type("S", (), {
            "symbol": "AMBIGUSDT", "asset_type": "crypto", "direction": "LONG", "score": 80,
            "entry_low": 99, "entry_high": 101, "stop_loss": 95, "take_profit_1": 105,
            "take_profit_2": 110, "risk_reward": 2, "technical_score": 80, "whale_score": 50,
            "sentiment_score": 50, "market_regime": "BULLISH"
        })())
        paper_trader.open_paper_trade(sid, "AMBIGUSDT", "LONG", 100, 95, 105, 110)
        self.assertEqual(paper_trader.mark_candle("AMBIGUSDT", {
            "time": datetime.now(timezone.utc).isoformat(), "high": 106, "low": 94, "close": 101
        }), 0)
        with __import__("sqlite3").connect(self.tmp.name) as con:
    ...
    def test_expired_trade_is_not_counted_as_win_or_sl(self):
        journal.init_db()
        sid = journal.record_setup(type("S", (), {
            "symbol": "EXPUSDT", "asset_type": "crypto", "direction": "LONG", "score": 80,
            "entry_low": 99, "entry_high": 101, "stop_loss": 95, "take_profit_1": 105,
            "take_profit_2": 110, "risk_reward": 2, "technical_score": 80, "whale_score": 50,
            "sentiment_score": 50, "market_regime": "BULLISH"
        })())
        ...
        with __import__("sqlite3").connect(self.tmp.name) as con:
            con.execute("UPDATE paper_trades SET opened_at=?,last_price=? WHERE signal_id=?", (old.isoformat(), 101, sid))
            con.commit()
    ...
    def test_binance_flow_is_optional_and_neutral_when_providers_fail(self):
        ...
        with patch("trade_bot.binance_flow._get", side_effect=fail):
    ...
    def test_binance_flow_can_confirm_long(self):
        responses = {
            "/api/v3/depth": {"bids": [[100, 10]], "asks": [[100, 5]]},
            "/fapi/v1/openInterest": {"openInterest": "100"},
            "/futures/data/openInterestHist": [
                {"sumOpenInterest": "100"}, {"sumOpenInterest": "104"}, {"sumOpenInterest": "104"}
            ],
            "/fapi/v1/fundingRate": [{"fundingRate": "0.0001"}],
            "/futures/data/takerlongshortRatio": [{"buySellRatio": "1.15"}],
        }
        def fake_get(base, path, params=None):
            return responses[path]
        with patch("trade_bot.binance_flow._get", side_effect=fake_get):
            result = snapshot("BTCUSDT", "LONG")
        self.assertTrue(result["available"])
        self.assertGreater(result["score"], 50)
        self.assertGreater(result["bias"], 0)

    def test_binance_flow_conflict_does_not_create_a_signal(self):
        responses = {
            "/api/v3/depth": {"bids": [[100, 1]], "asks": [[100, 10]]},
            "/fapi/v1/openInterest": {"openInterest": "100"},
            "/futures/data/openInterestHist": [
                {"sumOpenInterest": "100"}, {"sumOpenInterest": "104"}, {"sumOpenInterest": "104"}
            ],
            "/fapi/v1/fundingRate": [{"fundingRate": "0.0001"}],
            "/futures/data/takerlongshortRatio": [{"buySellRatio": "0.75"}],
        }
        def fake_get(base, path, params=None):
            return responses[path]
        with patch("trade_bot.binance_flow._get", side_effect=fake_get):
