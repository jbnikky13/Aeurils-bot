...
async def gems(update, context):
    try: await update.message.reply_text(_fmt_discovery("💎 SMALL-CAP GEM WATCH",discover_small_caps(10))+"\n\n⚠️ Discovery only — high risk; not a buy recommendation.")
    except Exception as exc: await update.message.reply_text(f"⚠️ Gem scan unavailable: {exc}")
async def perf(update, context):
    stats=performance(); await update.message.reply_text("📈 SIGNAL JOURNAL\n\n"+"\n".join(f"{k}: {v}" for k,v in stats.items()))
