...
class FuturesBacktestTests(unittest.TestCase):
    ...
    def test_runner_can_capture_move_after_tp2(self):
        ...
        rows=[]
        for i,close in enumerate([97,95,92,90,88]):
            ts=entry+pd.Timedelta(hours=i+1)
            rows.append({"time":ts,"open":close+1,"high":close+1,"low":close-1,"close":close,"atr":1})
        result=simulate(t,pd.DataFrame(rows))
        self.assertIn(result.outcome,{"WIN_RUNNER","EXPIRED"})
