...
def build_setup(symbol: str, asset_type: str, price: float, technical_score: int, whale_score: int, sentiment_score: int, technical_bias: float, whale_bias: float, atr: float) -> Signal:
    ...
    if direction == "WAIT" or score < 70:
        return Signal(symbol, asset_type, "WAIT", score, None, None, None, None, None, None, technical_score, whale_score, sentiment_score, ["No sufficiently strong multi-factor setup."], "Wait for confirmation.")
    ...
    return Signal(symbol, asset_type, direction, score, entry_low, entry_high, stop, tp1, tp2, reward / risk if risk else None, technical_score, whale_score, sentiment_score, [f"Technical score: {technical_score}/100", f"Whale-flow score: {whale_score}/100", f"Sentiment score: {sentiment_score}/100"], f"Invalid if price breaks the {direction.lower()} stop-loss level.")
