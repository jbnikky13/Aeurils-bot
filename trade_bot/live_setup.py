...
def _entry_context(df):
    x=enrich(df).dropna(subset=['ema20','atr']); r=x.iloc[-1]
...
def _crypto_futures_candles(symbol, interval, limit=240):
    """Prefer USD-M perpetual candles; fall back to spot only when futures data is unavailable."""
    try:
        return crypto_futures_klines(symbol, interval=interval, limit=limit), 'FUTURES'
    except Exception:
        return crypto_klines(symbol, interval=interval, limit=limit), 'SPOT_FALLBACK'


def _confirm_1h(symbol, direction, continuation_mode=False):
    """Use 1H as confirmation, with a continuation exception for extreme RSI."""
    try:
        df,_source=_crypto_futures_candles(symbol,'1h',240)
        ...
        if direction=='LONG':
            ...
            healthy=adx>=14 and (rsi<75 or continuation_mode)
        elif direction=='SHORT':
            aligned=bias<=-0.10 and minus>=plus-0.05
            healthy=adx>=14 and (rsi>25 or continuation_mode)
        else: return False, score, bias, ['No directional setup']
        return bool(aligned and healthy),score,bias,reasons+[f'1H ADX {adx:.1f}',f'1H RSI {rsi:.1f}',f'1H source: {_source}']
    except Exception as exc:
        return False,0,0,[f'1H confirmation unavailable: {type(exc).__name__}']


def _flow_ok(direction, flow):
    """Futures flow is supporting evidence; only severe conflict vetoes a setup."""
...
async def crypto_setup(symbol: str):
    """Generate from a completed 4H USD-M futures candle, then confirm on 1H."""
    df,source=_crypto_futures_candles(symbol,'4h',240)
    ...
    # Determine a provisional direction without applying normal entry/RSI vetoes;
    # this lets the futures engine see a genuine continuation candidate.
    provisional_direction=direction_from_components(tbias,0.0)
    flow={'available':False,'score':50,'bias':0,'reasons':['Futures flow not queried because no directional candidate was found.']}
    continuation=assess_continuation(enrich(signal_df),provisional_direction,flow)

    if provisional_direction in ('LONG','SHORT'):
        flow=binance_flow(symbol,provisional_direction)
        continuation=assess_continuation(enrich(signal_df),provisional_direction,flow)
    ...
    if preliminary.direction in ('LONG','SHORT') and not _flow_ok(preliminary.direction,flow):
        ...
        preliminary.reasons.append('Trade rejected: Binance futures flow shows a severe conflict with the 4H setup.')
        preliminary.invalidation='Wait for futures flow to align with the 4H direction.'
