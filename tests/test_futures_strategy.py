...
class FuturesStrategyTests(unittest.TestCase):
    ...
    def test_continuation_detects_strong_short_even_with_oversold_rsi(self):
        result=assess(self._continuation_df(),"SHORT",{"oi_change_pct":3.0,"taker_ratio":0.80,"liquidation_pressure":True}); self.assertIn(result.mode,{"BREAKOUT","CONTINUATION"}); self.assertGreaterEqual(result.score,70); self.assertTrue(result.trend_aligned); self.assertTrue(result.momentum_confirmed); self.assertTrue(result.flow_confirmed)

    def test_continuation_bypasses_normal_rsi_and_extension_veto(self):
        signal=build_setup("BRUSDT","crypto",120,85,80,50,-0.8,-0.5,5,"BEARISH",ema20=100,recent_high=105,recent_low=100,rsi=20,strategy_mode="BREAKOUT",continuation_score=85); self.assertEqual(signal.direction,"SHORT"); self.assertTrue(signal.runner_enabled); self.assertEqual(signal.entry_status,"READY_CONTINUATION"); self.assertGreaterEqual(signal.risk_reward,1.5)

    def test_normal_oversold_short_is_still_blocked(self):
        signal=build_setup("TESTUSDT","crypto",120,85,80,50,-0.8,-0.5,5,"BEARISH",ema20=100,recent_high=105,recent_low=100,rsi=20); self.assertEqual(signal.direction,"WAIT"); self.assertEqual(signal.entry_status,"EXTENDED")
    ...
    def test_futures_flow_uses_futures_depth(self):
        responses={"/fapi/v1/depth":{"bids":[[100,10]],"asks":[[100,5]]},"/fapi/v1/openInterest":{"openInterest":"100"},"/futures/data/openInterestHist":[{"sumOpenInterest":"100"},{"sumOpenInterest":"104"}],"/fapi/v1/klines":[[0,"100","101","99","100","1",0,0,0,0,0,0],[1,"100","101","95","96","1",0,0,0,0,0,0]],"/fapi/v1/fundingRate":[{"fundingRate":"0.0001"}],"/futures/data/takerlongshortRatio":[{"buySellRatio":"0.80"}],"/fapi/v1/allForceOrders":[]}
    ...
    def test_runner_keeps_trade_open_after_tp1(self):
        ...
        try:
            journal.DB=tmp.name; paper_trader.DB=tmp.name; journal.init_db(); sid=journal.record_setup(type("S",(),{"symbol":"RUNUSDT","asset_type":"crypto","direction":"SHORT","score":80,"entry_low":99,"entry_high":101,"stop_loss":105,"take_profit_1":95,"take_profit_2":90,"risk_reward":2,"technical_score":80,"whale_score":80,"sentiment_score":50,"market_regime":"BEARISH","strategy_mode":"CONTINUATION","continuation_score":80,"runner_enabled":True,"trailing_atr_multiplier":2.0})()); paper_trader.open_paper_trade(sid,"RUNUSDT","SHORT",100,105,95,90,runner_enabled=True,trailing_atr_multiplier=2.0); self.assertEqual(paper_trader.mark_candle("RUNUSDT",{"time":"2099-01-01T01:00:00+00:00","high":99,"low":94,"close":96,"atr":2}),0)
